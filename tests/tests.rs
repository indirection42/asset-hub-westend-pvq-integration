...
fn setup_pool_for_paying_fees_with_foreign_assets(
    ...
    let pool_liquidity: Balance = existential_deposit
        .max(foreign_asset_id_minimum_balance)
        .mul(100_000);
    ...
    assert_ok!(ForeignAssets::mint(
        ...
        pool_owner.clone().into(),
        (foreign_asset_id_minimum_balance + pool_liquidity)
            .mul(2)
            .into(),
    ));
    ...
    ExtBuilder::<Runtime>::default()
        ...
        .execute_with(|| {
            ...
            let unused_asset = trader
                .buy_weight(weight, payment.into(), &ctx)
                .expect("Expected Ok");
...
fn test_buy_and_refund_weight_with_swap_foreign_asset_xcm_trader() {
    ExtBuilder::<Runtime>::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                actual_refund,
                (foreign_location.clone(), asset_refund).into()
            );
    ...
    ExtBuilder::<Runtime>::default()
        ...
        .execute_with(|| {
            ...
            let asset: Asset = (
                ...
                asset_amount_needed + asset_amount_extra,
            )
                .into();
            ...
            let unused_assets = trader
                .buy_weight(bought, asset.into(), &ctx)
                .expect("Expected Ok");
...
fn test_asset_xcm_take_first_trader_with_refund() {
    ExtBuilder::<Runtime>::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(
                trader.buy_weight(bought, asset.into(), &ctx),
...
fn test_pvq_api_works() {
    ...
    ExtBuilder::<Runtime>::default()
        ...
        .execute_with(|| {
            ...
            // test quote_price_exact_tokens_for_tokens
            let mut args = vec![0u8];
            args.extend_from_slice(
                &(
                    native_location.encode(),
                    asset_1_location.encode(),
                    10 * UNITS,
                    false,
                )
                    .encode(),
            ...
            args.extend_from_slice(&(native_location.encode(), asset_1_location.encode()).encode());
...
fn authorized_aliases_work() {
    ExtBuilder::<Runtime>::default()
        ...
        .execute_with(|| {
            ...
            let alice_on_sibling_para = Location::new(
                1,
                [
                    Parachain(42),
                    AccountId32 {
...
asset_test_utils::include_asset_transactor_transfer_with_pallet_assets_instance_works!(
    ...
    xcm::v5::Location {
        parents: 1,
        interior: [
            xcm::v5::Junction::Parachain(1313),
            xcm::v5::Junction::GeneralIndex(12345)
        ]
        .into()
    },
...
fn receive_reserve_asset_deposited_roc_from_asset_hub_rococo_fees_paid_by_pool_swap_works() {
    ...
    let foreign_asset_id_location = xcm::v5::Location::new(
        2,
        [xcm::v5::Junction::GlobalConsensus(
            xcm::v5::NetworkId::ByGenesis(ROCOCO_GENESIS_HASH),
        )],
    );
...
fn governance_authorize_upgrade_works() {
    ...
    assert_err!(
        parachains_runtimes_test_utils::test_cases::can_governance_authorize_upgrade::<
            Runtime,
            RuntimeOrigin,
        >(GovernanceOrigin::Location(Location::new(
            1,
            Parachain(12334)
        ))),
